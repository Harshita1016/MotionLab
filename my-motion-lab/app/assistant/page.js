...
import AnimationPreviewCompact from "@/component/AnimationPreviewCompact"
...
export default function AssistantPage() {
  ...
  const [messages, setMessages] = useState([
    {
      role: "assistant",
      content: "Hi! I can help you with animation code and motion suggestions. Try asking for a fade-in effect or keyframes.",
    },
  ])
  ...
  const chatPanel = (
    <div className="space-y-4">
      {messages.map((msg, i) => (
        <ChatMessage key={i} role={msg.role === "user" ? "user" : "assistant"} content={msg.content}>
          {msg.code && (
            <div className="mt-3">
      ...
      <form onSubmit={handleSubmit} className="pt-4">
        <div className="flex gap-3">
          <input
            ...
            className="flex-1 px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-muted focus:outline-none focus:border-primary"
          />
          <button
            type="submit"
    ...
    <div className="rounded-2xl bg-card border border-white/10 p-6">
      ...
      <ul className="space-y-2">
        {examplePrompts.map((prompt, i) => (
          <li key={i} className="text-white text-sm px-3 py-2 rounded-lg bg-white/5 border border-white/10">
            {prompt}
          </li>
        ))}
      </ul>
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-black via-[#0f0f1a] to-black text-text">
      ...
      <div className="max-w-6xl mx-auto px-6 py-12">
        {/* Heading row: title + description left, animation preview right */}
        <div className="flex flex-col lg:flex-row lg:items-start gap-8 mb-12">
          <div className="flex-1 min-w-0">
            <h1 className="text-3xl md:text-4xl font-bold text-white mb-2">AI Assistant</h1>
            <p className="text-muted text-lg">
              Ask for animation code, keyframes, or motion ideas.
            </p>
          </div>
          <AnimationPreviewCompact />
        </div>
        ...
        <section className="mt-16 grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
          <div className="rounded-2xl bg-card border border-white/10 p-8">
            <h2 className="text-2xl font-bold text-white mb-2">Meet your Motion Assistant</h2>
            <p className="text-muted mb-6">
              Simply describe the animation you want in plain English. Our AI understands motion design and generates the perfect animation parameters for you.
            </p>
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 p-8 flex flex-col justify-center">
            <h3 className="text-xl font-semibold text-white mb-4">Why use AI for motion?</h3>
            <p className="text-muted text-sm leading-relaxed mb-4">
              Describe animations in natural language and get production-ready keyframes and parameters. No guesswork—our AI suggests easing, duration, and transforms that match your intent.
            </p>
            <p className="text-muted text-sm leading-relaxed">
              Perfect for designers and developers who want to ship polished animations without memorizing every CSS or Framer Motion option.
            </p>
          </div>
        </section>
